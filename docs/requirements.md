...
11.2. If the bug is not reproducible, the staff member or administrator must add a comment when marking it as “not reproducible”. The status of the bug must change to “not reproducible”.

11.3. If the bug is already fixed, the staff member or administrator must specify the version of the software when the bug was fixed. The status of the bug must change to “already fixed”.

11.4. If the bug is not really a bug (by design), the staff member or administrator must add a comment when marking it as “not a bug”. The status of the bug must change to “not a bug”.
